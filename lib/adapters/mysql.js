...
exports.mysql = Mysql = utility.define(null, {
    instance : {
        ...
        format : function(sql, params) {
            return this.db.format(sql, params)
        },
        ...
        _set : function(vals) {
            ...
            if (sqlObject.update) {
                ...
                if (typeof vals == "object" && !(vals instanceof Array)) {
                    ...
                    sqlObject.set += this.db.format(set.join(","), values);
                } else {
...
//            this.db.connect();
            var self = this;
            this.db.query(this.sql, hitch(this, function(err, results) {
                if (err) {
                    this.close();
                    console.error(err);
                    promise.errback(err);
    ...
    static : {
        createTable : function(table, db, callback) {
        ...
        dropTable : function(table, db, callback) {
            var sql = "DROP TABLE IF EXISTS " + table.tableName;
            //db.connect();
            db.query(sql, function(err) {
                callback(err || true);
            });
...
exports.mysql.types = require("./types/mysql").types;
