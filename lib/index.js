var Client = require('mysql').Client, dataset = require("./dataset"), model = require("./model"), adapters = require("./adapters"), util = require("./util"), hitch = util.hitch, promise = require("./promise"), Promise = promise.Promise, PromiseList = promise.PromiseList, plugins = require("./plugins"), migration = require("./migrations"), Table = require("./table").Table;

/*
 * options = { host = 'localhost'; port = 3306; user = null; password = null;
 * database = ''; flags = Client.defaultFlags; maxPacketSize = 0x01000000;
 * charsetNumber = Client.UTF8_UNICODE_CI; debug = false; }
 */
...
var Moose = util.define(migration, {
	instance : {
		 ...
		 * 
		 * options - minClients - minimum number of clients - db - the db to
		 * connect to - user - user - password - password
		 */
		...
		/*
		 * Adds a model to moose if a Table object is passed in then then model
		 * is returned immediatly otherwise the loading is deferred and a
		 * promise is returned TODO may abstract this out to be different
		 * methods?
		 * 
		 */
		...
		getSchema : function(tableName) {
			...
			if (tableName in this.schemas) {
				...
				throw new Error("Schema with " + tableName
						+ " is not created yet");
			}
			return m;
		},
		...
		getModel : function(tableName) {
			...
			if (this.models[tableName]) {
				...
				throw new Error("Model with " + tableName
						+ " is not created yet");
			}
			return m;
		}
