...
exports.Table = util.define(null, {
    instance : {
        ...
        constructor : function(tableName, properties) {
            ...
            var foreignKey = properties.foreignKey;
            var foreignKeys = properties.foreignKeys;
            delete properties.primaryKey;
            delete properties.foreignKeys;
    ...
    getters : {
        ...
        alterTableSql : function() {
            ...
            this.__newName && (sql += " " + " RENAME " + this.__newName);
            ...
            addColumnsSql.length && (sql += addColumnsSql.join(","),needComma = true);
            ...
            alterColumnsSql.length && (sql += (needComma ? ", " : " ") + alterColumnsSql.join(","),needComma = true);

            if (this.dropPrimaryKeySql) sql += (needComma ? ", " : " ") + this.dropPrimaryKeySql,needComma = true;
            if (this.addPrimaryKeySql) sql += (needComma ? ", " : " ") + this.addPrimaryKeySql,needComma = true;
            if (this.foreignKeys.length) sql += (needComma ? ", " : " ") + this.foreignKeys.join(","),needComma = true;
            if (this.uniqueSql.length) sql += (needComma ? ", " : " ") + this.uniqueSql.join(","),needComma = true;
            sql += ";";
            return sql;
        },
