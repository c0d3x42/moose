...
exports.Table = util.define(null, {
    ...
    getters : {
        ...
        alterTableSql : function() {
            ...
            addColumnsSql.length && (sql += ", " + addColumnsSql.join(","));
            if (dropColumns.length) {
                for (i in dropColumns) {
                    dropColumnsSql.push(adapters[this.type].dropColumn(dropColumns[i]));
                }
                sql += ", " + dropColumnsSql.join(",");
            }
            for (i in alteredColumns) {
                alterColumnsSql.push(adapters[this.type].alterColumn(i, alteredColumns[i]));
            }
            alterColumnsSql.length && (sql += ", " + alterColumnsSql.join(","));

            if (this.dropPrimaryKeySql) sql += ", " + this.dropPrimaryKeySql
            if (this.addPrimaryKeySql) sql += ", " + this.addPrimaryKeySql
            if (this.foreignKeys.length) sql += ", " + this.foreignKeys.join(",");
            if (this.uniqueSql.length) sql += ", " + this.uniqueSql.join(",");
            sql += ";";
            return sql;
        },
